...
export function Navbar() {
  ...
  return (
    <header className="flex h-16 shrink-0 items-center justify-between border-b border-black/10 bg-black/[0.02] px-6">
      <div className="text-sm font-medium text-aurora-text/60">{tenant ? tenant.name : "Platform Administration"}</div>

      <div className="flex items-center gap-3">
        <button className="rounded-lg p-2 text-aurora-text/60 transition hover:bg-black/10 hover:text-aurora-text" aria-label="Notifications">
          <Bell size={18} />
        </button>
        <button className="rounded-lg p-2 text-aurora-text/60 transition hover:bg-black/10 hover:text-aurora-text" aria-label="Help">
          <HelpCircle size={18} />
        </button>
